...
# Every suite under tests/ (2383 unittest cases; all pass, no PYTHONPATH needed)
...
├── OPEN_PROBLEMS.json            24 open problems, machine-readable
...
├── PRINCIPLES.json               11 recurring failure shapes, 43 instances
...
`PRINCIPLES.json` compresses ~70 findings across this archive into **11
recurring failure shapes** with 43 instances. Several turned up in files
...
The index classifies **151 claim ids by what can make each one FAIL**:
43 FALSIFIER, 68 NAMED_IN_TEST, 40 PROSE. So **111 of 151 have something
executable pointed at them** and 40 are written down with nothing that can
...
whether it holds, and why not when it does not. **68 of 151 claims are
recorded; the other 83 are reported as unregistered rather than given
...
deleted rather than learned from. 39 of 68 carry salvage today.
...
from). **Three more have reach ≥ 2 and nothing catches them.** *Test the
distinguishing operation* is now at **reach 3** — GIES-2, KEA-7 and GLY-3, the
same blindness in three formalisms that share no code: `outer(v,v)` losing the
sign, a Keating energy exactly even in the displacement, and a `sub_glyphs`
field that does not move when every magnitude in the reading changes. The
gap-mode mass criterion is at 2, having killed the $10k Er search and the
phosphorus local-mode claim in one stroke. **`enumerate-reachable-outputs`** is
the new one, also at 2: it killed `BLOCKAGE` by inspection and diagnosed the
phase-space matcher's 5-of-12, and unlike the other two it looks mechanisable —
sweep a classifier's input domain, list the outputs nothing produces, and make
the author say which absences are intended. `graveyard.py todo` is that list,
...
and `enumerate-reachable-outputs` each have reach 2 inside a single family, so
they are proven-but-untravelled; only three screens appear in more than one
family at all.
...
families matrix is **sparse on purpose and that is the finding**: 21 screens
over 17 families, only 3 used in more than one. Each screen carries an
...
CLAIMS_REGISTER.json            The statement, whether it holds, and why not
                                ...
                                establish. 68 of 151 recorded; the rest are
