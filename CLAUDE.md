...
python tests/test_repo_guard.py         # Null harness / symmetry veto / reach / collisions / prose / licence-ref + claim-status markers (74 tests, no deps)
...
python tests/test_octree_tolerance.py    # OT-1..5: the octree's tolerance knob, and the F1 record (6 tests, needs numpy)
...
python harness/run.py --impl py_octree_tol --resolutions 16 --tolerances 3.0,2.0,1.5,1.0,0.7,0.5,0.4,0.3   # the tolerance knob
...
| Repo guard | `tests/test_repo_guard.py` | 74 | Null harness verdicts incl. CLAIM_FAILS, symmetry veto hits/silence, instrument reach bands, stage 4: identical file bodies, screen names with two definitions, both directions; stage 5: each prose check shown firing and silent on a built tree, the real tree's four licence surfaces required to agree, and the audit shown to change nothing on disk; licence-ref markers counted apart from mismatches with both defects caught; claim-status markers counted apart from unsupported Nx claims, a bare `[refuted]` a defect, an unmarked claim still firing beside a marked one |
...
| Octree tolerance | `tests/test_octree_tolerance.py` | 6 | OT-1..5: the default rule reaches no resolution argument (the F1 record); the leaf count is monotone in the tolerance; a tolerance above the root estimate leaves one cell; a source cell reaches max_depth (which caught the one-sided estimator: `|f_k − f_c| / f_c` is capped at 1 when the centre is the closest point to a source, so the source cell never refined); the measured probe error falls as the tolerance tightens, scored with the harness's own metric |
...
├── spatial_grid.py               Adaptive octree: distance rule (no knob, F1) or error_tol rule (OT-1..5)
...
├── py_octree_tol/                The same octree with SpatialGrid(error_tol=...): refine while the
│                                 estimated local error exceeds the tolerance. knob: tolerance. Keeps
│                                 the per-leaf evaluation so the refinement change is measured alone.
...
SELECTION.md                    GENERATED by harness/run.py, never hand-edited. Axes are conditions:
                                ...
                                A tolerance-swept section for knob=tolerance impls, the held
                                workloads with what un-holds each, and the matched-accuracy table,
                                the one ratio that is a speedup.
...
- **The octree has no refinement parameter, and the knob that was built for it shows placement buying nothing per point on space-filling fields.** Three findings kept apart in README "Performance" because they have three causes. **F1, structural:** `SpatialGrid`'s only rule refines by distance to the nearest source under a depth cap; no resolution or accuracy argument reaches it, so its error is flat across resolutions 8 to 128 (dipole 0.174, quadrupole 0.106, wire 0.128/0.146) and every same-resolution comparison compares unequal accuracies. `SpatialGrid(error_tol=t)` is the knob: refine while the estimated local error, the largest symmetric ratio of a `Σ|q|/r²` proxy at the eight corners to its value at the centre, exceeds `t`. Its first version used `|f_k − f_c| / f_c`, which is capped at 1 whenever the centre is the closest point to a source, so the cell holding a source read err < 1 and never refined while its neighbours did; `tests/test_octree_tolerance.py` OT-4 caught it before the sweep ran. **F2, scoped:** with the knob, error moves with tolerance (0.49 → 0.11 on the dipole over 3.0 → 0.3), and at equal error the tolerance octree needs **4 to 14× more points** than the uniform grid (dipole 41,546 vs 5,832; quadrupole 65,248 vs 6,859; wire 29,184 vs 6,859 at tolerance 0.3), because the probes are uniform over the box and the rule spends its leaves within a few cells of each source where no probe lands. That holds for smooth single-scale fields; sparse, thin-layer and two-scale fields are UNMEASURED and specced in `harness/workloads_held.json`, held until the knob existed so the sparse regime is not tested under F1's confound. **F3, implementation:** one numpy call per leaf (ENG-5) makes the shipped octree 30–60× slower at equal error (0.016x–0.032x, `harness/matched_accuracy.py`) and the tolerance octree 0.001x–0.006x, which is F2's point ratio times F3's per-point penalty. `harness/run.py --tolerances` sweeps the knob, `SELECTION.md` carries the tolerance section and the held workloads, and manifests now declare `knob`.
