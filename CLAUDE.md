...
python tests/test_repo_guard.py         # Null harness / symmetry veto / reach / collisions / prose / licence-ref, claim-status and benchmark markers (76 tests, no deps)
...
python tests/test_harness_contract.py    # Implementation contract, matched accuracy, weighted probes, ceiling, HC-1..8 (38 tests, no deps)
python tests/test_octree_tolerance.py    # OT-1..7: both tolerance criteria, the depth cap reported, the F1 record (12 tests, needs numpy)
...
# an Nx performance claim with no benchmark nearby and no claim-status marker (see
# PROSE_AUDIT.md), a fenced shell command whose path is not in the tree, second-person
# address to the author, a filename with a space.
python repo_guard.py
python repo_guard.py prose             # stage 5 alone; PROSE_AUDIT.md is its dated snapshot
...
python harness/run.py --impl py_octree_tol --resolutions 16 --tolerances 3.0,2.0,1.5,1.0,0.7,0.5,0.4,0.3   # the tolerance knob (error criterion)
python harness/run.py --impl py_octree_mag --resolutions 16 --tolerances 3.0,2.0,1.5,1.0,0.7,0.5,0.4,0.3   # the first knob (magnitude proxy), for comparison
python harness/run.py --impl py_octree_tol --resolutions 16 --tolerances 0.25,0.2,0.15,0.12,0.1,0.08,0.06,0.05,0.04,0.03 --repeats 1 --stop-at-ceiling   # the ceiling question
...
| Repo guard | `tests/test_repo_guard.py` | 76 | Null harness verdicts incl. CLAIM_FAILS, symmetry veto hits/silence, instrument reach bands, stage 4: identical file bodies, screen names with two definitions, both directions; stage 5: each prose check shown firing and silent on a built tree, the real tree's four licence surfaces required to agree, and the audit shown to change nothing on disk; licence-ref markers counted apart from mismatches with both defects caught; claim-status markers counted apart from unsupported Nx claims, a bare `[refuted]` a defect, an unmarked claim still firing beside a marked one; `<!-- benchmark: path -->` counted apart and a path not in the tree a defect |
...
| Implementation contract | `tests/test_harness_contract.py` | 38 | HC-1..8: every manifest field required and the name tied to its folder; a versioned spec with seeded probes; the pure-Python reference reproduces the spec's Coulomb and current-element definitions; every status first-class and never blank; SELECTION.md renders NOT_RUNNABLE with its reason, counts NOT_MEASURED at the top and contains no best, rank or winner; a failing, hanging or malformed implementation becomes a record; harness/ imports only the standard library; HC-7: the matched-accuracy ratio interpolates a power-law sweep exactly, a target outside the sweep's error range is NOT_BRACKETED on the named side, a flat or rising sweep is NON_MONOTONE, and the committed SELECTION.md carries the section its committed records render; HC-8: the source-weighted probe set (B2) is seeded, sits at 3x+ the uniform set's 1/r² weight, and is scored beside the uniform set with an unanswered set scoring None, not 0; `finish(extra=)` reaches the record; the ceiling target is uniform_grid@128's E per workload |
| Octree tolerance | `tests/test_octree_tolerance.py` | 12 | OT-1..7: the default rule reaches no resolution argument (the F1 record); the leaf count is monotone in the tolerance; a tolerance above the root estimate leaves one cell; a source cell reaches max_depth (which caught the one-sided estimator: `|f_k − f_c| / f_c` is capped at 1 when the centre is the closest point to a source, so the source cell never refined); the measured probe error falls as the tolerance tightens, scored with the harness's own metric; OT-6: criterion=error's `field_at` reproduces `contract.reference_field` to 1e-9 on both probe sets, monotone leaf count, an unknown criterion refused; OT-7: leaves still above tolerance at max_depth are counted, flagged and at max_depth, zero when the root satisfies the tolerance, and the magnitude criterion caps too |
...
├── spatial_grid.py               Adaptive octree: distance rule (no knob, F1), or error_tol with criterion error | magnitude; depth_capped reported (OT-1..7)
...
repo_guard.py                   Stage 5.5 — the null stage. Five mechanical
                                checks (null harness, symmetry veto, reach,
                                collisions, prose) plus the human checklist for
                                circular targets and units. Stage 5 points the
                                apparatus at the repo's own README and fails the
                                run on hits; it reports and fixes nothing.
PROSE_AUDIT.md                  Dated snapshot of `python repo_guard.py prose`:
                                every hit with file and line, for the fixing pass.
...
│                                 interpolation and reads the wall ratio there. The shipped octree
│                                 lands at 0.016x-0.032x, i.e. 30-60x SLOWER than a uniform grid of
│                                 about the same point count (the harness benchmark); the tolerance
│                                 octree's rows are in SELECTION.md. Targets are grouped by their
│                                 knob (resolution or tolerance). stdlib.
├── workloads.json                The workloads and the CONDITIONS each declares (sparsity, scale)
├── workloads_held.json           SPECCED, NOT BUILT: sparse_point, boundary_layer, two_scale. Each
│                                 validates as a workload and names why it is held, what running it
│                                 settles, and what un-holds it. Never run; counted in SELECTION.md.
...
│                                 knob (resolution | tolerance | none: which axis the harness sweeps).
├── py_octree/                    The Engine's adaptive octree path, as a thin adapter (numpy). knob: none
├── py_octree_tol/                SpatialGrid(error_tol=t, criterion="error"): split while the field at any
│                                 child centre differs from the cell-centre field by more than t, relative
│                                 to the child's; bounded near a source, so the depth cap terminates it and
│                                 depth_capped travels in the record. knob: tolerance. Same per-leaf evaluation.
├── py_octree_mag/                The FIRST knob, criterion="magnitude": a sum|q|/r² proxy that diverges at a
│                                 point source, so refinement there is unbounded by construction. Kept so the
│                                 error criterion is reported against it at the same tolerances. knob: tolerance.
└── uniform_grid/                 The Engine's uniform grid, as a PEER implementation, not a reference. knob: resolution
SELECTION.md                    GENERATED by harness/run.py, never hand-edited. Axes are conditions:
                                ...
                                Every cell carries E/B on the uniform probes AND Ew/Bw on the
                                source-weighted probes (B2). A tolerance-swept section with
                                DEPTH_CAPPED(n) per cell, the held workloads with what un-holds each,
                                the ceiling verdict per workload, and the matched-accuracy table over
                                all four components, the one ratio that is a speedup.
...
integrations/gods-eye-view/     Integration map with the gods-eye-view fork (live globe: ADS-B, AIS,
                                ...
                                on great-circle synthetics; the measurement on real fixes is UNRUN
                                because both feeds were unreachable from the session. av-det-1 was
                                measured in the fork (src/data/labelArbiterNull.test.mjs): the label
                                weights are not flat and move ~3% of labels, both halves pinned.
```
...
- **The octree has no refinement parameter; the knob built for it refines on an error estimate; and the metric decides who wins.** Three findings kept apart in README "Performance". **F1, structural:** `SpatialGrid`'s only rule refines by distance to the nearest source under a depth cap; no argument reaches it, so its error is flat across resolutions 8 to 128. **The knob:** `SpatialGrid(error_tol=t, criterion="error")` splits a cell while the field at any child centre differs from the cell-centre field by more than t, relative to the child's, using the real field. Bounded near a source, so the depth cap terminates it and `depth_capped` is reported as a first-class count. The first knob (`criterion="magnitude"`, kept as `py_octree_mag`) refined on a Σ|q|/r² proxy that diverges at a point source, so its refinement there was unbounded by construction; at tolerance 0.3 on the dipole it spent 41,546 points with 1,825 capped leaves where the error criterion spends 8,471 with 188. Its own first version was one-sided, `|f_k − f_c| / f_c`, capped at 1 when the centre is the nearest point to a source, so the source cell never refined; OT-4 caught it before the sweep ran. One degeneracy is recorded: the quadrupole's field at the box centre is zero by symmetry, so the root estimate is exactly 1 and any tolerance ≥ 1 leaves one cell reporting E = 1.000. **F2, scoped, settled two ways:** every record now carries the error on 256 uniform probes AND on 256 source-weighted probes (density ∝ Σ1/r², the cells an adaptive grid refines). At equal error the shipped octree needs 1.24–1.52× the uniform grid's points on uniform probes and 0.60–1.04× on weighted probes; the error-criterion octree 3–22× on uniform probes and 0.88–8.9× on weighted, closing toward parity as the tolerance tightens. The octree loses on one metric and wins on the other, and that is the result; the harness reports both rather than choosing. Sparse, thin-layer and two-scale fields are UNMEASURED and specced in `harness/workloads_held.json`. **Ceiling:** sweeping the tolerance down, the point cap (128³ = 2,097,152) comes before the resolution-128 grid's uniform-probe error on every workload (dipole 3,166,626 points at E 0.0187 vs 0.0158; wire 2,880,431 at 0.0134 vs 0.0093; quadrupole NOT_REACHED at the sweep's end, 2,082,032 points at E 0.0170 vs 0.0094, 0.7% under the cap); on weighted probes the dipole octree reaches the grid's Ew at 933,199 points against 1,061,208. **F3, implementation:** one numpy call per leaf (ENG-5) makes every octree 25–100× slower at equal error on either metric, larger than any per-point gain F2 shows. `harness/run.py --tolerances --stop-at-ceiling`, `harness/matched_accuracy.py` over four components, manifests declare `knob`.
