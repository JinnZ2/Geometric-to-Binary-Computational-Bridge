...
python tests/test_gi_network.py          # Geometric network + phi resonance, GI/GR/GB (102 tests, no deps)
...
# Every suite under tests/ (2485 unittest cases; all pass, no PYTHONPATH needed)
...
python geometric_intelligence/falsifiers_gi_network.py   # GI-1..16, GR-1..6, GB-1..5
...
| Geometric network + φ resonance | `tests/test_gi_network.py` | 102 | GI-1..16, GR-1..6, GB-1..5: measurements that do not propagate, an unsatisfiable threshold clause, a design contradiction resolved in the measurement's favour, non-monotonic band edges, an unread constructor parameter, and a 15.7% false-alarm rate on noise |
...
├── OPEN_PROBLEMS.json            29 open problems, machine-readable
...
├── PRINCIPLES.json               11 recurring failure shapes, 54 instances
...
`PRINCIPLES.json` compresses ~100 findings across this archive into **11
recurring failure shapes** with 54 instances. Several turned up in files
...
whether it holds, and why not when it does not. **92 of 182 claims are
...
deleted rather than learned from. 63 of 92 carry salvage today.
...
CLAIMS_REGISTER.json            The statement, whether it holds, and why not
                                ...
                                establish. 92 of 182 recorded; the rest are
