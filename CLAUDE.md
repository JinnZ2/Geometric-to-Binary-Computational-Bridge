...
python tests/test_gi_network.py          # Geometric network + phi resonance, GI/GR (67 tests, no deps)
...
# Every suite under tests/ (2450 unittest cases; all pass, no PYTHONPATH needed)
...
python geometric_intelligence/falsifiers_gi_network.py   # GI-1..7, GR-1..6
...
| Geometric network + φ resonance | `tests/test_gi_network.py` | 67 | GI-1..7, GR-1..6: measurements that do not propagate, an unsatisfiable threshold clause, a design contradiction resolved in the measurement's favour, non-monotonic band edges, an unread constructor parameter, and a 15.7% false-alarm rate on noise |
...
├── falsifiers_gi_network.py      Runnable GI/GR report, exits nonzero
├── CLAIMS.md                     Generated by `python claims_index.py render`
└── network/                      phi-scaled network analysis (subpackage, stdlib)
    ├── core.py                     GeometricNetwork; RECONSTRUCTED, was not in the drop
    ├── integrity.py                IntegrityMonitor; AUDIT header carries GI-1..6
    ├── resonance.py                MultiScaleResonance; AUDIT header carries GR-1..6
    ├── README.md                   What runs, what is absent, and the import path
    └── FIELD_GUIDE.md              Cold-workshop procedures; three corrections marked
...
├── OPEN_PROBLEMS.json            27 open problems, machine-readable
...
├── PRINCIPLES.json               11 recurring failure shapes, 50 instances
...
`PRINCIPLES.json` compresses ~85 findings across this archive into **11
recurring failure shapes** with 50 instances. Several turned up in files
...
The index classifies **168 claim ids by what can make each one FAIL**:
58 FALSIFIER, 68 NAMED_IN_TEST, 42 PROSE. So **126 of 168 have something
executable pointed at them** and 42 are written down with nothing that can
...
`experiments/silicon_speculative/` — rendered
...
whether it holds, and why not when it does not. **80 of 168 claims are
recorded; the other 88 are reported as unregistered rather than given
...
deleted rather than learned from. 51 of 80 carry salvage today.
...
families matrix is **sparse on purpose and that is the finding**: 22 screens
over 19 families, only 5 used in more than one. Each screen carries an
...
CLAIMS_REGISTER.json            The statement, whether it holds, and why not
                                ...
                                establish. 80 of 168 recorded; the rest are
